...
\usepackage{amsmath}
\usepackage{hyperref}
% Make hyperlinks unobtrusive in the PDF
...
\begin{itemize}
    ...
    \item Nucleotide Transformer (HuggingFace): multi-species pretrained checkpoints (example: \url{https://huggingface.co/InstaDeepAI/nucleotide-transformer-v2-50m-multi-species}).
    \item Basenji (Calico Labs): reference implementation and codebase (\url{https://github.com/calico/basenji}).
    \item Basset (reference implementation / baseline): \url{https://github.com/davek44/Basset}.
    \item DeepSEA resources and portal: model and data documentation (\url{http://deepsea.princeton.edu/help/}).
    \item Optional ready-to-use model hubs (Kipoi): model cards and variant-effect wrappers (e.g., \url{https://kipoi.org/models/DeepSEA/predict/}, \url{https://kipoi.org/models/DeepSEA/variantEffects/}).
\end{itemize}
...
\midrule
DNABERT-2 & \texttt{zhihan1996/DNABERT-2-117M} (HuggingFace) \\
Nucleotide Transformer v2 & \texttt{InstaDeepAI/nucleotide-transformer-v2-50m-multi-species} (HuggingFace) \\
Probe (linear head) & \texttt{notebooks/results/checkpoints/probe\_best.pt} (repo) \\
Basset / Basenji baseline & \texttt{notebooks/results/bassetmini\_best.pt} or Basenji reference implementation (GitHub) \\
\bottomrule
\end{tabular}
\end{table}
