...
% Additional practical notes to complete layout on page 6
\paragraph{Data availability and license}
Processed datasets used in the experiments (train/val/test splits and preprocessed NPZ files) are included in the `data/processed/` directory of the repository. Raw source references and coordinate files are provided under `data/raw/`. All code in the repository is released under the MIT license; see `LICENSE` for full terms.

\paragraph{Contact and support}
For questions about reproducing results or for access to intermediate artifacts not present in the repository, please open an issue on the project GitHub or contact the author at \texttt{angel.morenu@ufl.edu}.
