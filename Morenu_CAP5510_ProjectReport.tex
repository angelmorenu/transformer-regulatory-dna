...
Understanding how DNA sequence encodes regulatory function is one of the central challenges in computational genomics. Regulatory elements such as enhancers, promoters, and DNase-accessible regions control transcriptional programs and cellular identity.
...
\section{Methods}
\subsection{Datasets}
All datasets originate from the DeepSEA training bundle, ENCODE, and Roadmap epigenomics tracks (Zhou and Troyanskaya, 2015; DeepSEA portal; ENCODE Project; Roadmap Epigenomics). The processed data include:
\begin{itemize}
...
\begin{itemize}
    \item DNABERT-2 (HuggingFace): pretrained checkpoints and tokenizers (example: \url{https://huggingface.co/zhihan1996/DNABERT-2-117M}) (Nguyen et al., 2023).
    \item Nucleotide Transformer (HuggingFace): multi-species pretrained checkpoints (example: \url{https://huggingface.co/InstaDeepAI/nucleotide-transformer-v2-50m-multi-species}) (Dalla-Torre et al., 2023).
    \item Basenji (Calico Labs): reference implementation and codebase (\url{https://github.com/calico/basenji}) (Calico Labs, Basenji GitHub).
    \item Basset (reference implementation / baseline): codebase on GitHub (\url{https://github.com/davek44/Basset}) and original paper (Kelley et al., 2016).
    \item DeepSEA resources and portal: model and data documentation (portal: \url{http://deepsea.princeton.edu/help/}) (DeepSEA portal; Zhou and Troyanskaya, 2015).
    \item Optional ready-to-use model hubs (Kipoi): model cards and variant-effect wrappers (e.g., \url{https://kipoi.org/models/DeepSEA/predict/}, \url{https://kipoi.org/models/DeepSEA/variantEffects/}) (Kipoi model hub).
\end{itemize}

Where available we used HuggingFace checkpoints as starting points for probes and finetuning; open-source Basenji/Basset code served as lightweight CNN baselines for direct implementation and runtime comparisons.
...
Spearman: \VepDeepSeaSpearman{} (p=\VepDeepSeaSpearmanP{}). Top-K: K=10 overlap \VepDeepSeaTopK10Overlap{} (fold \VepDeepSeaTopK10Fold{}, p=\VepDeepSeaTopK10P{}); K=25 overlap \VepDeepSeaTopK25Overlap{} (fold \VepDeepSeaTopK25Fold{}, p=\VepDeepSeaTopK25P{}); K=50 overlap \VepDeepSeaTopK50Overlap{} (fold \VepDeepSeaTopK50Fold{}, p=\VepDeepSeaTopK50P{}).
...
% (intentionally removed duplicate sentence)
...
\begin{itemize}
    ...
    \item The transformer context window used in most experiments is limited (512 bp in practice), which reduces sensitivity to regulatory elements acting at longer ranges (Avsec et al., 2021).
...
\begin{itemize}
    \item Conceptualization and project design: A.M.
    \item Implementation: data preprocessing, CNN baseline, transformer probe, training harness, and VEP pipeline: A.M.
    \item Experiments and analysis: model training runs, variant-effect analyses, plotting, and statistical summaries: A.M.
    \item Writing and visualization: manuscript text, figures, and result interpretation: A.M.
    \item Reproducibility artifacts: notebooks, scripts, and environment specification included in repository: A.M.
\end{itemize}
