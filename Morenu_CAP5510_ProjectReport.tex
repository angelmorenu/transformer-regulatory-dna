...
\begin{itemize}
    \item DNABERT-2 (HuggingFace): pretrained checkpoints and tokenizers (example: \url{https://huggingface.co/zhihan1996/DNABERT-2-117M}) [1].
    \item Nucleotide Transformer (HuggingFace): multi-species pretrained checkpoints (example: \url{https://huggingface.co/InstaDeepAI/nucleotide-transformer-v2-50m-multi-species}) [6].
    \item Basenji (Calico Labs): reference implementation and codebase (\url{https://github.com/calico/basenji}) [7].
    \item Basset (reference implementation / baseline): codebase on GitHub (\url{https://github.com/davek44/Basset}) and original paper [4].
    \item DeepSEA resources and portal: model and data documentation (portal: \url{http://deepsea.princeton.edu/help/}) [8,3].
    \item Optional ready-to-use model hubs (Kipoi): model cards and variant-effect wrappers (e.g., \url{https://kipoi.org/models/DeepSEA/predict/}, \url{https://kipoi.org/models/DeepSEA/variantEffects/}) [9].
\end{itemize}
...
\begin{itemize}
    ...
    \item The transformer context window used in most experiments is limited (512 bp in practice), which reduces sensitivity to regulatory elements acting at longer ranges [2].
