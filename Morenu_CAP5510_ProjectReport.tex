...
\usepackage{cite}
\usepackage{hyperref}
% left-align captions (avoid centered TABLE titles)
...
\section{Methods}
\subsection{Datasets}
All datasets originate from the DeepSEA training bundle, ENCODE, and Roadmap epigenomics tracks \cite{zhou2015deepsea, deepsea_portal, encode_project, roadmap_portal}. The processed data include:
\begin{itemize}
...
\subsection{Models and sources}
For model selection and baseline comparisons I used publicly available pretrained models and reference implementations. Primary sources include:
\begin{itemize}
    \item DNABERT-2 (HuggingFace): pretrained checkpoints and tokenizers (example: \url{https://huggingface.co/zhihan1996/DNABERT-2-117M}) \cite{nguyen2023dnabert2}.
    \item Nucleotide Transformer (HuggingFace): multi-species pretrained checkpoints (example: \url{https://huggingface.co/InstaDeepAI/nucleotide-transformer-v2-50m-multi-species}) \cite{dallatorre2023nucleotide}.
    \item Basenji (Calico Labs): reference implementation and codebase (\url{https://github.com/calico/basenji}) \cite{basenji_github}.
    \item Basset (reference implementation / baseline): codebase on GitHub (\url{https://github.com/davek44/Basset}) and original paper \cite{kelley2016basset}.
    \item DeepSEA resources and portal: model and data documentation (portal: \url{http://deepsea.princeton.edu/help/}) \cite{deepsea_portal, zhou2015deepsea}.
    \item Optional ready-to-use model hubs (Kipoi): model cards and variant-effect wrappers (e.g., \url{https://kipoi.org/models/DeepSEA/predict/}, \url{https://kipoi.org/models/DeepSEA/variantEffects/}) \cite{kipoi_models}.
\end{itemize}

Where available I used HuggingFace checkpoints as starting points for probes and finetuning; open-source Basenji/Basset code served as lightweight CNN baselines for direct implementation and runtime comparisons.

% Short reproducibility table listing exact checkpoints used in experiments
\begin{table*}[!t]
\centering
\caption{Model checkpoints and sources used in experiments (examples).}
\label{tab:checkpoints}
\begin{tabular}{l p{6.3cm} p{2.5cm}}
\toprule
...
Where possible, variant-effect scores were benchmarked against published DeepSEA variant scoring and related workflows by computing rank correlations and enrichment of high-impact sites; these comparisons help contextualize absolute delta magnitudes and support cross-study interpretation.
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.34\textheight,keepaspectratio]{notebooks/results/plots/metrics_bar_auroc_test.png}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.36\textheight,keepaspectratio]{notebooks/results/plots/metrics_bar_prauc_all.png}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.40\textheight,keepaspectratio]{notebooks/results/plots/vep_heatmap_test.png}
    \caption{Variant effect heatmap across positions (double-column view).}
    \label{fig:vep}
\end{figure*}
 
% DeepSEA comparison summary (auto-generated snippet)
\input{notebooks/results/plots/vep_deepsea_summary.tex}
\noindent\textbf{Comparison to DeepSEA (summary):} \\
% The macros above are generated by `scripts/write_vep_deepsea_tex.py`.
\ifnum\VepDeepSeaAvailable=1
Spearman: \VepDeepSeaSpearman{} (p=\VepDeepSeaSpearmanP{}). Top-K: K=10 overlap \VepDeepSeaTopKTenOverlap{} (fold \VepDeepSeaTopKTenFold{}, p=\VepDeepSeaTopKTenP{}); K=25 overlap \VepDeepSeaTopKTwentyFiveOverlap{} (fold \VepDeepSeaTopKTwentyFiveFold{}, p=\VepDeepSeaTopKTwentyFiveP{}); K=50 overlap \VepDeepSeaTopKFiftyOverlap{} (fold \VepDeepSeaTopKFiftyFold{}, p=\VepDeepSeaTopKFiftyP{}).
\else\ifnum\VepDeepSeaAvailable=2
Processed-test fallback: \VepDeepSeaFallbackSamples{} samples, max score \VepDeepSeaFallbackMaxScore{}.
\else
No DeepSEA comparison available (see \texttt{notebooks/results/plots/vep_deepsea_summary.tex}).
\fi\fi
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.38\textheight,keepaspectratio]{notebooks/results/plots/topk_bar_test.png}
...
\subsection*{Figure files used}
The figures included in this report are stored under \texttt{notebooks/results/plots/} and correspond to the following files (each referenced in-text by its figure number):
\begin{itemize}
    \item \texttt{notebooks/results/plots/metrics\_bar\_auroc\_test.png} (Figure~\ref{fig:auroc}) -- aggregated AUROC across splits.
    \item \texttt{notebooks/results/plots/metrics\_bar\_prauc\_all.png} (Figure~\ref{fig:prauc}) -- aggregated PR-AUC across splits.
    \item \texttt{notebooks/results/plots/vep\_heatmap\_test.png} (Figure~\ref{fig:vep}) -- variant effect heatmap for test sequences.
    \item \texttt{notebooks/results/plots/topk\_bar\_test.png} (Figure~\ref{fig:topk}) -- top-50 variant effects bar plot.
\end{itemize}

These files are committed in the repository so the report can include the figures during build. If any figure is missing, regenerate the corresponding notebook or run the plotting script in \texttt{notebooks/05\_results\_visualization.ipynb}.
...
Feasibility note: to keep experiments tractable I leveraged publicly available pretrained checkpoints (HuggingFace DNABERT-2 and Nucleotide Transformer) and limited fine-tuning to short windows (1 kb) in most experiments. Heavy finetuning runs were reserved for HPC nodes or Colab Pro when a GPU was required; lightweight baselines (Basset/Basenji) and frozen linear probes enabled practical runtime and memory comparisons on single-GPU or workstation setups.

Code and data: the full repository (code, processed inputs, notebooks, and results) is available at \url{https://github.com/angelmorenu/transformer-regulatory-dna}.
...
\begin{itemize}
    ...
    \item The transformer context window used in most experiments is limited (512 bp in practice), which reduces sensitivity to regulatory elements acting at longer ranges \cite{avsec2021enformer}.
...
\begin{itemize}
    ...
    \item PR-AUC being higher than AUROC in the experiments typically indicates that the model ranks positives more effectively than it separates classes uniformly in imbalanced regulatory datasets; PR-AUC is often more informative.
...
Planned next steps include: (1) full fine-tuning of DNABERT-2 over longer windows to assess long-range effects, (2) cross-cell-type transfer experiments to measure generalization, and (3) integrating complementary signals (e.g., chromatin accessibility) to improve VEP calibration. I welcome collaboration and feedback — please open an issue or pull request on the project repository if you reproduce these results or want to extend the experiments.
\fussy

\section*{Contributions}
The following contributions summarize the author's work on this project and may be used for attribution or grading:
\begin{itemize}
    \item Conceptualization and project design.
    \item Implementation: data preprocessing, CNN baseline, transformer probe, training harness, and VEP pipeline.
    \item Experiments and analysis: model training runs, variant-effect analyses, plotting, and statistical summaries.
    \item Writing and visualization: manuscript text, figures, and result interpretation.
    \item Reproducibility artifacts: notebooks, scripts, and environment specification included in repository.
\end{itemize}

% small nudge to help final page balancing if needed
\enlargethispage{2\baselineskip}

\section*{APPENDIX: REPRODUCIBILITY AND HYPERPARAMETER RANGES}

To facilitate reproducibility and document the ranges explored during development, Table~\ref{tab:hyperparams} summarizes the primary hyperparameter sweeps used throughout tuning and ablation experiments. These ranges reflect GPU memory limits, training stability observations, and validation feedback.

\begin{table}[h]
\caption{Hyperparameter ranges used during tuning and ablation.}
\label{tab:hyperparams}
\centering
\begin{tabular}{p{2.2cm} p{2cm} p{3.6cm}}
\hline
        \textbf{Parameter} & \textbf{Range} & \textbf{Notes} \\
\hline
Learning rate (probe head) & $1\times 10^{-5}$ -- $1\times 10^{-3}$ & Log-uniform sweep, default $5\times10^{-4}$ \\
...
\subsection*{A. Quick Reproduce Commands}
...
python src/train_transformer.py \
    --config runs/transformer_finetune/run_config.json
...
\subsection*{B. Data Availability and Licensing}

Processed datasets (train/val/test NPZ files) are included in \texttt{data/processed/}. Raw references and coordinate files are stored under \texttt{data/raw/}. All code is released under the MIT license; see \texttt{LICENSE} for full terms.

\subsection*{C. Contact and Support}

For questions or access to additional artifacts, open a GitHub issue at: \texttt{https://github.com/angelmorenu/transformer-regulatory-dna}.
...
\bibitem{nguyen2023dnabert2}
N.~Nguyen, D.~Tran, \emph{et al.},
``DNABERT-2: Efficient foundation model for DNA language,''
bioRxiv, 2023.

\bibitem{avsec2021enformer}
Z.~Avsec, H.~Zhou, J.~Kelley, \emph{et~al.},
``Effective gene expression prediction from sequence by integrating long-range interactions,''
\emph{Nature Methods}, vol. 18, pp. 1196--1203, 2021.
...
\bibitem{basenji_github}
Calico Labs, ``Basenji -- GitHub repository,'' \url{https://github.com/calico/basenji} (accessed Nov 2025).

\bibitem{deepsea_portal}
DeepSEA Project, ``DeepSEA Web Portal and help pages,'' \url{http://deepsea.princeton.edu/help/} (accessed Nov 2025).

\bibitem{kipoi_models}
Kipoi community, ``Kipoi model hub (DeepSEA models),'' \url{https://kipoi.org/models/DeepSEA/} (accessed Nov 2025).

\bibitem{encode_project}
ENCODE Project Consortium, ``ENCODE Project Portal,'' \url{https://www.encodeproject.org/} (accessed Nov 2025).

\bibitem{roadmap_portal}
NIH Roadmap Epigenomics, ``Roadmap Epigenomics Data Portal (WashU),'' \url{https://egg2.wustl.edu/roadmap/web_portal/} (accessed Nov 2025).
