...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\textwidth]{notebooks/results/plots/metrics_bar_prauc_all.png}
    \caption{Aggregated PR-AUC across all splits.}
    \label{fig:prauc}
\end{figure}

% small hyperparameters table to document tuning
\begin{table}[H]
\centering
\caption{Key hyperparameters used in representative runs.}
\label{tab:hyper}
\begin{tabular}{lcc}
	oprule
Model & LR & Epochs\\
\midrule
Basset & $1\times10^{-3}$ & 30\\
Transformer probe & $5\times10^{-4}$ & 10\\
\bottomrule
\end{tabular}
\end{table}
